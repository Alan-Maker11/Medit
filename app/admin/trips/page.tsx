...
export default async function TripsPage() {
  ...
  const [{ data: trips, error: tripsError }, { data: drivers }] = await Promise.all([
    supabase
      .from("trips")
      .select("id, date, time, client_name, trip_type, status, total_fare, driver_id, needs_wheelchair, needs_stair_climber, payment_status, services(name), drivers(name), vehicles(name)")
